...
from sentiment_new import SentimentClassifier
...
class GPReviewDataset(Dataset):
    ...
    def __init__(self, reviews, targets, positions, tokenizer, max_len):
...
for epoch_num in range(int(args.num_train_epochs)):
    ...
    for step, batch_data in enumerate(train_data_loader):
        positions = batch_data["position"]
        input_ids = batch_data["input_ids"].to(device)
        attention_mask = batch_data["attention_mask"].to(device)
        targets = batch_data["targets"].to(device)
        outputs = model(input_ids=input_ids, positions = positions, attention_mask=attention_mask, label_ids = targets)
    ...
    model.eval().cpu()
    ckpt_model_filename = "new_ckpt_epoch_" + str(epoch_num+1) + ".pth"
    ckpt_model_path = os.path.join(args.output_dir, ckpt_model_filename)
    torch.save(model.state_dict(), ckpt_model_path)
    model.to(device)
        

    with open(os.path.join(args.output_dir, "new_test_ep_"+str(epoch_num+1)+".txt"), "w") as f_test:
...
save_model_filename = "new_epoch_" + str(args.num_train_epochs) + "_" + timestamp + ".model"
